...
router.post('/login', function (req, res, next) {
    ...
    mysqls.query('SELECT * FROM user where ID=?', [ID], function (error, result) {
        ...
        if (PW == result[0].PWD) {
            ...
            res.redirect('http://127.0.0.1:3000/receiving_quantity_graph');
        }
    });
});
    ...
    if (req.session.loginid == undefined) {
        if (year == undefined) {
            mysqls.query('SELECT * from monthly_receiving_volume where year = ?', ['2019'], function (error, result) {
                const data = {name: "방문자", power_check: 2, receive_graph_d: result};
                res.render('./CHECK_MANAGEMENT/receiving_quantity_graph.ejs', data);
            });
        } else {
