    ...
    if (req.session.loginid == undefined) {
        mysqls.query('SELECT * FROM warehousing limit ?,10', [start_page], function (error, result) {
            ...
            mysqls.query('SELECT * FROM warehousing limit ?,10', [start_page + 10], function (error, results) {
                ...
                res.render('./ITEM_ADMINSTRATION/Forward_management.ejs', data);
            });
        });
    } else {
        mysqls.query('SELECT * FROM warehousing limit ?,10', [start_page], function (error, result) {
...
router.post('/view', function (req, res) {
    ...
    if (req.session.loginid == undefined) {
        console.log(name + unique_number + quantity + forwarding_date);
        mysqls.query('SELECT * FROM warehousing where name = ? and unique_number = ? and quantity = ? and forwarding_date =?', [name, unique_number, quantity, forwarding_date], function (error, result) {
            if (error) {
                console.log(error);
            } else {
                const data = {name: '방문자', power_check: 2, forward_d: result};
                res.render('./ITEM_ADMINSTRATION/Forward_management_view.ejs', data);
            }
        });
    }
    else{
        ...
        mysqls.query('SELECT * FROM warehousing where name = ? and unique_number = ? and quantity = ? and forwarding_date =?', [name, unique_number, quantity, forwarding_date], function (error, result) {
            if (error) {
                ...
                const data = {name: req.session.loginid, power_check: 2, forward_d: result};
                res.render('./ITEM_ADMINSTRATION/Forward_management_view.ejs', data);
            }
        });
    }
...
router.get('/write', function (req, res) {
    ...
    res.render('./ITEM_ADMINSTRATION/Forward_management_write.ejs', data);
});
...
router.post('/write', function (req, res, err) {
    ...
    if(quantity1 == undefined){
        mysqls.query('insert into warehousing(large_category, small_category, name, unique_number, quantity, forwarding_date) values(?,?,?,?,?,?)', [large_category, small_category, name, unique_number, quantity2, forwarding_date], function (error, result) {
            if(error){
                console.log(error);
            }
            else{
        ...
        mysqls.query('insert into warehousing(large_category, small_category, name, unique_number, quantity, forwarding_date) values(?,?,?,?,?,?)', [large_category, small_category, name, unique_number, quantity1, forwarding_date], function (error, result) {
            if(error){
                console.log(error);
            }
            else{
...
router.get('/search', function (req, res, err) {
    ...
    if (item_name != "" && date == "") {
        ...
        if (req.session.loginid == undefined) {
            mysqls.query('SELECT * FROM warehousing where forwarding_date = ? limit ?,10', [date, start_page], function (error, result) {
                if (error) {
                    console.log('에러:' + error);
                }
                mysqls.query('SELECT * FROM warehousing where forwarding_date = ? limit ?,10', [date, start_page], function (error, results) {
                    if (error) {
                        console.log('에러:' + error);
                    }
                    check_page_number = results.length;
